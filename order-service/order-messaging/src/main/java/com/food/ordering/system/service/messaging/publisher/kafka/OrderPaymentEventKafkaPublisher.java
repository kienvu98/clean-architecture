...
import com.food.ordering.system.order.service.domain.ports.output.message.pulisher.payment.PaymentRequestMessagePublisher;
...
public class OrderPaymentEventKafkaPublisher implements PaymentRequestMessagePublisher {
    ...
    private final KafkaMessageHelper kakKafkaMessageHelper;

    public OrderPaymentEventKafkaPublisher(OrderMessagingDataMapper orderMessagingDataMapper,
                                           KafkaProducer<String, PaymentRequestAvroModel> kafkaProducer,
                                           OrderServiceConfigData orderServiceConfigData,
                                           KafkaMessageHelper kakKafkaMessageHelper) {
        this.orderMessagingDataMapper = orderMessagingDataMapper;
        this.kafkaProducer = kafkaProducer;
        this.orderServiceConfigData = orderServiceConfigData;
        this.kakKafkaMessageHelper = kakKafkaMessageHelper;
    }
    ...
    public void publish(OrderPaymentOutboxMessage orderPaymentOutboxMessage, BiConsumer<OrderPaymentOutboxMessage, OutboxStatus> outboxCallback) {
        OrderPaymentEventPayload orderPaymentEventPayload = kakKafkaMessageHelper
        ...
        try {
            ...
            kafkaProducer.send(orderServiceConfigData.getPaymentRequestTopicName(),
                    ...
                    kakKafkaMessageHelper.getKafkaCallback(orderServiceConfigData.getPaymentRequestTopicName(),
                            paymentRequestAvroModel,
                            orderPaymentOutboxMessage,
                            outboxCallback,
                            orderPaymentEventPayload.getOrderId(),
                            "PaymentRequestAvroModel"));
            log.info("OrderPaymentEventPayload sent to kafka for order id: {} and sagaId: {}",
                    orderPaymentEventPayload.getOrderId(), sagaId);
        } catch (Exception e) {
            log.error("Error while sending OrderPaymentEventPayload to kafka with order id: {} and saga id: {}",
                    orderPaymentEventPayload.getOrderId(), sagaId);
            throw new OrderDomainException("Error While sending OrderPaymentEventPayload to kafka with order id: "+
                    orderPaymentEventPayload.getOrderId() + " and saga id: " +  sagaId);
        }
    }
}
